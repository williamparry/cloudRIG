...
function createRemoteEncryptedFile(data, cb) {
	...
	sendMessage(cmd, function(err, tmpFileRef) {
		if(err) {
			cb(err);
			return;
		}
	...
	var params = {
		SpotFleetRequestConfig: {
			...
			LaunchSpecifications: [{
				...
				UserData: btoa("<powershell>" + getPS1("Configure.ps1") + "</powershell>"),
	...
	if(settings.fromBase) {
		...
		startFunctions = startFunctions.concat([
			...
			function(instance, cb) {
				...
				sendMessage(getPS1("Create-Folder.ps1"), function () { cb(null, instance); });
			},
			...
			function(instance, cb) {
				...
				setTimeout(function() { cb(null, instance); }, 3000);
			},

			function(instance, cb) {
				reporter.report("Installing Open SSL");
				sendMessage(getPS1("Install-OpenSSL.ps1"), function () { cb(null, instance); });
			},

			function(instance, cb) {
				reporter.report("Getting Public Key");
				sendMessage(getPS1("Get-PublicKey.ps1"), function(err, publicKey) {
