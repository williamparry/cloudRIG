...
function transferEBSVolume(volume, cb) {
	...
	const IOPScoef = 50;
	const minVolumeSize = 4;
	const minIOPS = 100;
	const maxIOPS = 32000;
	const newVolumeSize = volume.Size > minVolumeSize ? volume.Size : minVolumeSize;
	const modifyVolumeParams = {
		...
		function checkModify(data, callback) {
			...
			ec2.describeVolumesModifications({
				...
				if (data && data.VolumesModifications) {
					const modify = data.VolumesModifications[0];
