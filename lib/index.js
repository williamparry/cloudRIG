...
function clearStartupLambdas(cb) {
	async.eachLimit(getStartupLambdaNames(), 1,
		(folderName, cb) => {
		...
		getStartupLambdaNames().forEach(folderName => {
			if (
				!data.Topics.some(topic => {
					return topic.TopicArn.includes(folderName) &&
						topic.TopicArn.includes(config.AWSRegion);
				}
				)
			) {
				found = false;
		...
		getStartupLambdaNames().forEach(folderName => {
			if (
				!data.Functions.some(func => {
					return func.FunctionArn.includes(folderName) &&
						func.FunctionArn.includes(config.AWSRegion);
				}
				)
			) {
				found = false;
			...
			if (!startLambdas || !startSNS) {
				...
				questions.push({
					q: "Shall I create a series of lambdas and SNS topics for startup? Their names are:\n"
						+ lambdaNames,
					m: updateLambdas.bind(this, userID)
				});
				startLambdas = true;
			}
...
function logStartup(cb, startTime) {
	...
	function check() {
		...
		async.eachLimit(groups, 1, (group, eachcb) => {
