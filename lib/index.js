...
function start(cb) {
	...
	var isCancelled;
	var cancelledError =
		"Sorry, there wasn't enough capacity for your specification. Try again later.";
	...
	var _checkMarkedForStop = (function() {
		...
		function check() {
			getSpotInstanceRequests(function(err, instanceRequests) {
				if (
					instanceRequests[0] &&
					instanceRequests[0].Status.Code === "marked-for-stop"
				) {
					clearTimeout(timer);
					timer = null;
					_cancel();
	...
	var _checkSpotPrice = function(cb) {
		getAvailabilityZoneInfo(function(err, data) {
			...
			if (
				parseFloat(zoneInfo.SpotPrice) >= parseFloat(config.AWSMaxPrice)
			) {
				cb(
					"Your max price is too low right now. Either wait for the cost to go down, or raise your max price"
				);
				return;
			}
	...
	var _getBestAvailabilityZone = function(cb) {
		getBestAvailabilityZone(function(err, bestAvailabilityZone) {
			...
			if (
				parseFloat(bestAvailabilityZone.SpotPrice) >=
				parseFloat(config.AWSMaxPrice)
			) {
				cb(
					"Your max price is too low right now. Either wait for the cost to go down, or raise your max price"
				);
	...
	var _request = function(cb) {
		ec2.runInstances(
			...
			function(err, data) {
				...
				rollback.add(
					deleteSpotInstance.bind(null, instance.InstanceId)
				);
				rollback.add(
					deleteSpotRequest.bind(null, instance.SpotInstanceRequestId)
				);
	...
	var _configureGraphics = function(cb) {
		...
		sendMessage(getPS1("Configure-Graphics.ps1"), function(err, data) {
			...
			if(typeof data === "string" && data.trim() === "ok") {
				reporter.report("Graphics up-to-date")
				cb()
			} else {
				reporter.report("Graphics drivers updated")
				_reboot(function() {
	...
	getState(function(err, currentState) {
		...
		getAMI(function(err, ami) {
			...
			if (ami) {
				...
				reporter.report(
					"Making your cloudRIG! This happens once and may take a while."
				);
