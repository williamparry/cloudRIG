	...
	async.parallel(
		...
		function(err, results) {
			...
			if (ret.activeInstance && ret.instanceReady) {
				async.parallel([getScheduledStop, getRemainingTime], function(err, results) {
	...
	getSetup(function(err, results) {
		...
		if (!cloudRIGRole) {
			...
			if (!startLambdas || !startSNS) {
				...
				questions.push({
					q:
						"Shall I create a series of lambdas and SNS topics for startup? Their names are:\n" +
						lambdaNames,
					...
					async.series(
						[
							cb => {
								...
								zip.file(
									"common.js",
									fs.readFileSync(`${__dirname}/lambdas/cloudrigLambdaCommon/common.js`)
								);
