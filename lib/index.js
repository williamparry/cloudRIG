...
function updateLambdas(userId, cb) {
    ...
    async.eachLimit(fs.readdirSync(`${__dirname}/lambdas/`), 1,
        (folderName, cb) => {
            if (folderName.substring(0, 9) == "cloudrig-") {
                ...
                async.series([
                    cb => {
                        ...
                        fs.remove(`${__dirname}/lambdas/${folderName}/node_modules`, (err, data) => {
                            cb();
                        })
                    },
                    cb => {
                        // Install dependencies
                        exec(`cd "${__dirname}/lambdas/${folderName}" && yarn install`, (err, data) => {
                            cb();
                        })
                    },
                    cb => {
                        ...
                        archive.directory(`${__dirname}/lambdas/${folderName}`, false);
                        archive.finalize();
