...
function installEmulator(packageName, cb) {
	async.series([
		function (callback) {
			sendMessage(getPS1("Install-Chocolatey.ps1"), callback);
		},
		function (callback) {
			sendMessage(`choco install -f -y ${packageName}; return "ok"`, callback);
		}
	], function (err, data) {
		if (err) { cb(err); return }
		cb();
...
function getScheduledStop(cb) {
	...
	getActiveInstances(function (err, instances) {
		...
		if (instances.length > 0) {
			...
			sendMessage("Get-ScheduledTask -TaskName CloudRIGScheduledShutdown", function (err, data) {

				if (err) {
					cb(err);
					return;
				} else {
					cb(null, data.indexOf("----------ERROR-------") === -1)
				}
			})
	...
	getRemainingTime(function (err, data) {
		...
		async.parallel([
			...
			function (cb) {
				...
				sendMessage(getPS1("Schedule-Shutdown-Notification_tpl.ps1")
					.replace(/__CLOUDRIG_REASON__/, "scheduled")
					.replace(/__CLOUDRIG_REMAININGMINUTES__/, data.remainingMinutes - 2), cb)
			...
			function (cb) {
				...
				sendMessage(getPS1("Schedule-Shutdown_tpl.ps1")
					.replace(/__CLOUDRIG_REMAININGMINUTES__/, data.remainingMinutes), cb)
...
function start(cb) {
	...
	var _waitForSSM = function (cb) {
		...
		function check() {
			...
			console.log("describeInstanceInformation for :  " + JSON.stringify(state.instance))
...
function prepareUpdate(cb) {
	...
	getState((err, state) => {
		...
		if (settings.cloudRIGRole) {
			toClear.push(cloudrig.deleteAllRolesAndInstanceProfile);
		}

		if (settings.lambda) {
			toClear.push(cloudrig.deleteLambda);
		}

		if (settings.lambdaSave) {
			toClear.push(cloudrig.deleteLambdaSave);
		}
...
function setup(cb, isUpdate) {

	isUpdate = isUpdate || fs.existsSync(updateFlagFile);
	...
	async.parallel([
		...
		if (questions.length > 0 && isUpdate) {
			...
			reporter.report("Update completed");
