	...
	async.waterfall([
		...
		function(data, cb){
			...
			iam.createPolicy({
				...
				PolicyName: "eventAccessForInstances",
				Path: "/cloudRig/"
			}, cb);
		},
		function(data, cb) {
			reporter.report(`Attaching the policy eventAccessForInstances to ${cloudRIGInstanceProfileRoleName}`);
	...
	getRemainingTime(function(err, data) {
		...
		async.parallel([
			...
			if (err) cb(err);
				...
				sendMessage(getPS1("SetExTimeForNotify.ps1"), function(err, ssmResponse){
					if(err){
						cb(err);
					}
					else{
					sendMessage("C:\\ProgramData\\Amazon\\EC2-Windows\\Launch\\Apps\\CancelShutdownApp\\CancelShutdownApp.exe " +
					`${cloudStopRulePrefix}-${data.instanceId} ${cloudNotifyRulePrefix}-${data.instanceId} ${data.remainingMinutes}`, function(err, data){});
					cb();
					}
				})
			}
