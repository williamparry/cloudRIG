...
var RollbackHandler = function() {
	...
	return {
		...
		process: function(originalError, cb) {
			...
			async.series(steps, function(err, result) {
				...
				if(err) { 
					console.log("Error with rollback, please log into AWS console and check");
				} else {
...
function createEBSVolume(availabilityZone, fromVolumeId, cb) {
	...
	if(fromVolumeId) {
		...
		// TODO: Put snapshot into rollback
		ec2.createSnapshot({
...
module.exports = {
	...
	setup: function(_userDataReader, _userDataWriter, cb) {
		...
		async.parallel([
			...
			var mainUserId 			= results[3];
			var ipAddress 			= results[4];
			var lambda 				= results[5];
			var keyPair				= results[6];
