...
// Note this is loosely coupled in lambdaHandler.js
...
function getRequiredConfig() {
	return ["AWSCredentialsProfile", "AWSMaxPrice", "AWSRegion", "AWSAvailabilityZone", "ParsecServerId"];
}
...
function getRemainingTime(cb) {
	...
	getActiveInstances(function(err, instances) {
		...
		cb(null, {
			remainingMinutes: 59 - Math.floor(moment().diff(instances[0].LaunchTime, 'minutes', true) % 60),
		...
		async.parallel([
			...
			function(cb) {
				...
				cloudwatchevents.putRule({
					...
					ScheduleExpression: `rate(${data.remainingMinutes} minute${data.remainingMinutes > 1 ? 's' : ''})`,
					State: "ENABLED"
				}, function (err, ruleData) {
			...
			function(cb) {
				...
				var notifyMinutes = data.remainingMinutes - 1;

				cloudwatchevents.putRule({
					Name: cloudNotifyRulePrefix + "-" + data.instanceId,
					ScheduleExpression: `rate(${notifyMinutes} minute${notifyMinutes > 1 ? 's' : ''})`,
					State: "ENABLED"
				}, function (err, ruleData) {
