...
function lambdaFunction() {
	...
	function waterfall(arr, cb) {
		...
		function newImage() {
			...
			waterfall([
				function (cb) {
					console.log("Create new IAM for instance with id " + instanceId);
					ec2.createImage({
		...
		waterfall([
			...
			function (data, cb) {
				if (data.Images.length > 0) {
					if(data.Images[0].BlockDeviceMappings.length > 0){
						oldSnapshotId=data.Images[0].BlockDeviceMappings[0].Ebs.SnapshotId
					}
			...
			function(data, cb){
				if(data){
					...
					ec2.deleteSnapshot({SnapshotId:oldSnapshotId},cb);
				}
				else cb(null);
			}
		],
				...
				ec2.describeImages({
					...
					series([
						function (cb2) {
							...
							else cb2("Images.length less than 1");
						},
						function (cb2) {
							console.log("Deleting cloud watch rule");
							cloudwatchevents.deleteRule({
...
function deleteCloudrigRole(cb) {
		...
		function(cb) {
			...
			getInstanceProfiles(function(err, instanceProfiles) {
				...
				if(instanceProfiles.length > 0) {
					...
					var instanceProfileName = instanceProfiles[0].InstanceProfileName;
					...
					iam.removeRoleFromInstanceProfile({
						...
						deleteInstanceProfile(instanceProfileName, function() {
							if(err) { cb(err); return }
							cb();
						})
