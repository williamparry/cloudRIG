...
function getCredentials() {
	if(fs.existsSync(AWSCredsFile)) {
		return fs.readFileSync(AWSCredsFile)
	}
	return "";
}

function saveCredentialsFile(data) {
	fs.writeFileSync(AWSCredsFile + '-backup-' + +(new Date()), getCredentials());
	fs.writeFileSync(AWSCredsFile, data);
}
...
function getPendingAMI(cb) {
	...
	reporter.report("Finding Pending AMI");

	ec2.describeImages({
		Owners: ['self'],
		Filters: standardFilter
	}, function(err, data) {
		console.log(standardFilter, data)
		if (err) { cb(err); return; } 
	...
	setup: function(cb) {
		...
		async.parallel([
			...
			if(!cloudRIGRole || (cloudRIGRole && !cloudRIGRole.Role)) {
				questions.push({
					q: "Shall I make a role and instance profile called '" + cloudRIGRoleName + "' for handling the AWS services?",
