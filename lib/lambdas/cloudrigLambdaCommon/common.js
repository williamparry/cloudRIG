...
Common.prototype.report = function report(message) {
	console.log("[cloudrigrun" + this.storedStartTime + "] " + JSON.stringify(message));
	...
	sns.publish(
		{
			...
			TopicArn: "arn:aws:sns:" + obj.storedAwsRegion + ":" + obj.storeduserId + ":cloudrig-deleteRequestAndInstance"
		},
		function(err, data) {
			if (err) {
				obj.report(err);
...
Common.prototype.scheduleNextLambda = function scheduleNextLambda(rate, lambdaARNQueue, eventBody) {
	...
	if (lambdaARNQueue.length == 0) {
		...
		console.log("Triggering next lambda via cloudwatch rule in " + rate);
...
Common.prototype.start = function start(runCB, eventBody) {
	...
	// trigger obj lambda
	cloudwatchevents.disableRule({ Name: "ScheduleLambda" }, function(err, data) {
